const Featured = ({
  ...
  return (
    <>
      <div
        className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${isFeaturedContentExist ? "flex-row-reverse" : "flex-row"
          }`}
      >
        ...
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill transitions-theme ${isFeaturedContentExist
                ? "h-64 lg:h-60 md:h-56 sm:h-48 xsm:h-40 "
                : "h-80 lg:h-72 md:h-68 sm:h-52 xsm:h-44 "
